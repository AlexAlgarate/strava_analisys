...
class WeeklyActivitiesFetcher(IActivityFetcher):
    ...
    async def fetch_activity_data(self, previous_week: bool = False) -> Any:
        monday, sunday = helper.get_week_epoch_range(
        ...
        return await self.api.make_request(
            endpoint="/activities", params=params
        )
...
class DetailedActivitiesFetcher(IActivityFetcher):
    async def fetch_activity_data(
        ...
        activities = await WeeklyActivitiesFetcher(
            self.api
        ).fetch_activity_data(previous_week=previuos_week)
        if not activities:
        ...
        return [
            ...
            for activity in detailed_activity
        ]
    ...
    async def _fetch_all_activity_details(
        ...
        tasks = [
            self._get_activity_details(activity_id)
